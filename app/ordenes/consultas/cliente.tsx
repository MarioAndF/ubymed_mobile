...
export default function ClienteScreen () {
    ...
    const [selectedRow, setSelectedRow] = React.useState<{ label: string; value: string; }[] | null>(null);
    ...
    const example_user = { paciente_nombre: 'Mario', paciente_apellido: 'Flores', paciente_telefono: '4008-4336'};
    ...
    const handleRowSelect = (id: string) => {
        if (id === '0') {
            ...
            const selectedRow = Object.entries(example_user)
                .map(([key, value]) => ({
                    label: relevantFields[key as keyof typeof relevantFields],
                    value: value,
                }));
    ...
    const handleDataChange = (updatedData: { label: string; value: string }[]) => {
    ...
    const handleNext = () => {
        if (selectedRow) {
            const updatedData = selectedRow.reduce((acc, { label, value }) => {
                ...
                const key = Object.keys(relevantFields).find(k => relevantFields[k as keyof typeof relevantFields] === label);
                if (key) {
            ...
            router.push('/ordenes/consultas/motivo'); // Navegar a la siguiente pantalla
        }
    };
